...
mach_inject(
	...
	if (!err) {
		...
		err = vm_write(remoteTask, remoteCode, (pointer_t)image, (unsigned int)imageSize);
	...
	if (!err && paramBlock != NULL && paramSize) {
		...
		if (!err) {
			...
			err = vm_write(remoteTask, remoteParamBlock,
			               (pointer_t)paramBlock, (unsigned int)paramSize);
...
#if defined(__x86_64__)
		const struct mach_header_64 *header = (const struct mach_header_64 *)_dyld_get_image_header((unsigned int)imageIndex); // why no function that returns mach_header_64
				...
				if (fatHeader->magic == OSSwapBigToHostInt32(FAT_MAGIC)) {
					...
					for (archIndex = 0; archIndex < archCount; archIndex++) {
						...
						if (localArchInfo->cputype == cpuType) {
							...
							if (localArchInfo->cpusubtype == cpuSubtype)
								break;
						}
					}
